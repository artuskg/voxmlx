...
- 2026-02-09: User requested running newly-added incremental 10s scripts/config after pull, but fetched remote branches/history did not contain `scripts/incremental_file_eval.py`, `scripts/run_incremental_10s_matrix.py`, or `perf/incremental_10s_matrix.json`. Confusing because request referenced a “most recent commit” that was not reachable from available refs. Resolution: verify presence with `git ls-tree`/`git log --all --name-only` immediately after pull and ask for exact branch or commit SHA when filenames are absent.
- 2026-02-09: Verified requested incremental-10s files against `origin/codex/consolidated-post-e6d193e` and `origin/pull/1/head`; branch refs existed but filenames were still absent. Confusing because branch-name confirmation alone looked like the blocker was resolved. Resolution: after branch discovery, always verify file existence in that ref before promising command execution.
- 2026-02-09: Incremental 10s matrix runner failed against mixed historical refs for two reasons: (1) evaluator imported `voxmlx.constants` missing in baseline `e6d193e`, and (2) matrix used local branch names (`codex/...`) while only `origin/codex/...` refs were available. Resolution: for cross-ref benchmarks, add backward-compatible import fallbacks in eval scripts and normalize variant refs to remote-tracking names when local branches do not exist.
